...
public sealed class IngestionPipeline
{
    ...
    { _bronze = bronze; _validator = validator; _extractor = extractor; _chunker = chunker;
      _embedder = embedder; _search = search; _registry = registry; _opts = opts; }

    public async Task<IngestResult> IngestAsync(byte[] pdf, SdsMetadata meta, CancellationToken ct)
    {
        ...
        var validation = _validator.Validate(text, meta.Cas);
        ...
        await _registry.UpsertAsync(new RegistryPointer(registryId, meta.Cas, meta.Supplier, meta.ProductName,
            ...
            chunks.Select(c => c.Id).ToList(), now, null, meta.MasterListId, meta.SourceStrategy), ct);
