    ...
    IKnowledgeStore? knowledge = null, ICatalogLookup? catalog = null)
{
    ...
    private async Task OnConstraintsAsync(ConstraintsDoc c, CancellationToken ct)
    {
        ...
        //
        // This SUPERSEDES the earlier "park until the physicist's XRF lands" behaviour: the pool agent does not
        // need a measured background to propose from need, so the pipeline no longer stalls on an offline human
        // to even begin Discovery. The XRF measurement is still entered on Background (XrfEndpoints) — it writes
        // ElementPools, taking the branch below instead, and its measured background still gates the FINAL
        // result at Dosing through the detection floor. What is gone is only the wait to start.
    ...
    private async Task CloseProjectAsync(string projectId, CancellationToken ct)
    {
        ...
        // contract `First()`s. An exception that escaped here would hit the dispatch loop — which logs and
    ...
    private async Task TryDecideAsync(string projectId, CancellationToken ct)
    {
        ...
        try
        {
            ...
            // ArgumentException. Outside the try that throw escapes into the dispatch loop — which
