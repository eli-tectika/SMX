...
public sealed class MafAgent : ISmxAgent
{
        ...
        IRunTrail? trail = null)
    {
        ...
        // Tools are wrapped so each one writes its step AS IT COMPLETES. Harvesting them from the
        // finished response instead — what this used to do — delivered the whole trail in one burst
        // when the turn returned, which on a three-minute stage meant the operator watched a single
        // line for two minutes and then twenty-six steps at once. See TrailedFunction.
        _agent = new ChatClientAgent(chatClient, instructions: instructions, name: name,
            tools: TrailedFunction.Wrap(tools, Trail));
    }
    ...
    private sealed class AgentThreadAdapter(AIAgent agent, AgentSession session) : ISmxAgentThread
    {
        ...
        public async Task<string> SendAsync(string message, CancellationToken ct)
        {
            ...
            // Tool steps are NOT written here. Each tool writes its own as it completes
            // (TrailedFunction), so they reach the operator during the turn rather than after it.
            // Emitting them here as well would double every row.
