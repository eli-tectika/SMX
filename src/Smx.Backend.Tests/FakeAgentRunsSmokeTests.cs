...
public class FakeAgentRunsSmokeTests
{
    ...
    {
        var fake = new FakeAgentRuns();
        var reply = await ((Smx.Backend.Pipeline.IAgentRuns)fake).RunChatAsync(
            new Smx.Backend.Agents.ChatTools(new Smx.Domain.Tests.Fakes.InMemoryRecordStore(), "p1", Stages.Discovery, "k1"),
