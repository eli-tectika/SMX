...
public class DosingCostEndToEndTests : IClassFixture<WebApplicationFactory<Program>>
{
    ...
    public async Task TheWholeJourney_ToAPricedDosableCitedAnswer()
    {
        ...
        // 3. Re-enter the pipeline (in production the supervisor does this). There is no signature step in
        //    between any more — §16.4 deleted the regulatory gate, so nothing gates Dosing at all.
