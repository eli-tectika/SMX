...
{
    /// The VP hard gate — now the ONLY signature in the system (2026-08-06 redesign §16.4) — arms when the
    /// Decision stage has produced a code offer for EVERY component. "Selected" at ARM time means a
    /// ProposedCode is present: the agent's offer for the VP to confirm or override IN the signing call;
    /// the confirmation itself never gates arming, or the gate could only be signed after it was signed.
    ///
    /// THE REGULATORY-SIGNATURE BLOCKER IS GONE, with the gate that produced it. What it was really
    /// guarding — "does a human still owe this analysis a look" — did not go with it: the signing endpoint
    /// and both of its read surfaces consult <see cref="EvidenceReview.Outstanding"/>, which refuses while a
    /// live flagged verdict is unopened. That check outlived the signature because it never needed one.
    ///
    /// Blockers name things precisely — the absent decision, or the exact component with no code — because
    /// a blocker the operator cannot locate is a gate they cannot ever arm.
    public static (bool Ok, IReadOnlyList<string> Blockers) Armable(DecisionDoc? decision)
    {
    ...
    ///
    /// Upstream stages (Discovery/Regulatory) are deliberately not listed: a revision there replaces
    /// verdicts, and a replaced verdict comes back with `EvidenceReviewed=false`, so
    /// <see cref="EvidenceReview.Outstanding"/> refuses the pen for any non-Pass result the operator has not
    /// re-opened — that window has its own guard.
