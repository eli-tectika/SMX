...
**Official sources decide the law.** No regulatory verdict rests on an open web search: compliance draws
exclusively from a maintained list of official regulators — a substantive line of defence against treating
non-authoritative or outdated material as regulation. Candidate *discovery* may consult the open web to
surface markers beyond the internal catalogue, but only through a single anonymizing, controlled egress, and
a web-sourced candidate is flagged for validation and can never clear a compliance gate on that basis alone.
