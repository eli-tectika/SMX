...
- **Plan**: `Microsoft.Web/serverfarms` Flex Consumption (`sku: { tier:'FlexConsumption', name:'FC1' }`) — **both envs** (Flex is 1:1 app:plan, so one plan per app).
- **Two Function Apps** (`Microsoft.Web/sites@2023-12-01`, `functionapp,linux`), VNet-integrated on `snet-functions`, App Insights wired — **separate identities for security isolation**:
  ...
  - Each app has its **own** runtime storage (keyless + private endpoints) and its **own** Flex plan. Infra creates the **shells**; function code is published later.
