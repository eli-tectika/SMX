...
| D6 | Sweep host | **Plain `TimerTrigger`** (not Durable). | Brief §4 says "timer-triggered." The saturating due-set is small. (Reg-sync, added later, also uses a plain `TimerTrigger` + HTTP resume — no Durable — see infra §15 reconciliation note; if fan-out is ever needed Durable can be introduced.) |
