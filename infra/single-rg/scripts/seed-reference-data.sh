...
# Upload the reference workbooks to Bronze (lineage) and invoke SeedReferenceData (single-RG variant).
# Run AFTER publish-functions.sh + configure-auth.sh, BEFORE harden.sh (needs public reach + Entra token).
# Requires the caller to hold "Storage Blob Data Contributor" on the storage account.
RG="${1:-${SMX_RG:-Tectica}}"
...
# The RG holds three storage accounts (bronze medallion + two Flex-Consumption app storages);
