...
@description('gpt-5-mini deployment capacity (thousands of TPM). GlobalStandard TPM capacity is a RATE LIMIT, not a cost — the same lesson embeddingCapacity learned below. At 1 (=1K TPM) every agent turn died on HTTP 429 the moment the pipeline ran (live 2026-07-27): one RAG-shaped request carries more than 1K tokens of system prompt and retrieved context before it asks anything. 200 was still not enough: the regulatory stage fans out REGULATORY_PARALLELISM=4 ways over per-substance runs, each carrying its own retrieved corpus, and 2 of 8 children still 429d (live 2026-07-27). 800 of the region limit of 1000 covers the fan-out, and nothing else draws on this pool.')
param gpt5MiniCapacity int = 800
